...
STRATEGY_NAMES = {
    ...
    "amd_phase": {"ru": "AMD (накопл.→манип.→распред.)",
                  "uz": "AMD (to'plash→manip.→taqsim.)"},
...
REGEX = [
    ...
    (re.compile(r"AMD: swept accumulation low \(([\d.]+)\), distribution up expected"),
     r"AMD: снят минимум накопления (\1), ожидается распределение вверх",
     r"AMD: to'plash minimumi yig'ildi (\1), yuqoriga taqsimot kutiladi"),
    (re.compile(r"AMD: swept accumulation high \(([\d.]+)\), distribution down expected"),
     r"AMD: снят максимум накопления (\1), ожидается распределение вниз",
     r"AMD: to'plash maksimumi yig'ildi (\1), pastga taqsimot kutiladi"),
